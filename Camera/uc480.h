...
#   define UC480_VERSION_CODE   UC480_VERSION(4, 20, 0)
#endif
...
#define IS_CM_BGR5_PACKED           (IS_SET_CM_RGB15 | IS_CM_ORDER_BGR | IS_CM_FORMAT_PACKED) // occupies 16 Bit
...
#define IS_CM_RGB10_PACKED          (25 | IS_CM_ORDER_RGB | IS_CM_FORMAT_PACKED)
#define IS_CM_BGR10_PACKED          (25 | IS_CM_ORDER_BGR | IS_CM_FORMAT_PACKED)

#define IS_CM_RGB12_PACKED          (30 | IS_CM_ORDER_RGB | IS_CM_FORMAT_PACKED)              // occupies 48 Bit
#define IS_CM_BGR12_PACKED          (30 | IS_CM_ORDER_BGR | IS_CM_FORMAT_PACKED)              // occupies 48 Bit
#define IS_CM_RGBA12_PACKED         (31 | IS_CM_ORDER_RGB | IS_CM_FORMAT_PACKED)              // occupies 64 Bit
#define IS_CM_BGRA12_PACKED         (31 | IS_CM_ORDER_BGR | IS_CM_FORMAT_PACKED)              // occupies 64 Bit
...
// ----------------------------------------------------------------------------
  USBCAMEXP   is_SetVideoInput          (HCAM hCam, INT Source);
  USBCAMEXP   is_SetSaturation          (HCAM hCam, INT ChromU, INT ChromV);
  USBCAMEXP   is_SetHue                 (HCAM hCam, INT Hue);
  USBCAMEXP   is_SetVideoMode           (HCAM hCam, INT Mode);
  USBCAMEXP   is_SetAGC                 (HCAM hCam, INT Mode);
  USBCAMEXP   is_SetSyncLevel           (HCAM hCam, INT Level);
  USBCAMEXP   is_ShowColorBars          (HCAM hCam, INT Mode);
  USBCAMEXP   is_SetScaler              (HCAM hCam, float Scalex, float Scaley);
  USBCAMEXP   is_SetCaptureMode         (HCAM hCam, INT Mode);
  USBCAMEXP   is_SetHorFilter           (HCAM hCam, INT Mode);
  USBCAMEXP   is_SetVertFilter          (HCAM hCam, INT Mode);
  USBCAMEXP   is_ScaleDDOverlay         (HCAM hCam, BOOL boScale);
  USBCAMEXP   is_GetCurrentField        (HCAM hCam, int* pField);
  USBCAMEXP   is_SetVideoSize           (HCAM hCam, INT xpos, INT ypos, INT xsize, INT ysize);
  USBCAMEXP   is_SetKeyOffset           (HCAM hCam, INT nOffsetX, INT nOffsetY);
  USBCAMEXP   is_PrepareStealVideo      (HCAM hCam, int Mode, ULONG StealColorMode);
  USBCAMEXP   is_SetParentHwnd          (HCAM hCam, HWND hwnd);
  USBCAMEXP   is_SetUpdateMode          (HCAM hCam, INT mode);
  USBCAMEXP   is_OvlSurfaceOffWhileMove (HCAM hCam, BOOL boMode);
  USBCAMEXP   is_GetPciSlot             (HCAM hCam, INT* pnSlot);
  USBCAMEXP   is_GetIRQ                 (HCAM hCam, INT* pnIRQ);
  USBCAMEXP   is_SetToggleMode          (HCAM hCam, int nInput1, int nInput2, int nInput3, int nInput4);
  USBCAMEXP   is_SetDecimationMode      (HCAM hCam, INT nMode, INT nDecimate);
  USBCAMEXP   is_SetSync                (HCAM hCam, INT nSync);
  // only FALCON duo/quattro
  USBCAMEXP   is_SetVideoCrossbar       (HCAM hCam, INT In, INT Out);
  // watchdog functions
  USBCAMEXP   is_WatchdogTime           (HCAM hCam, long lTime);
  USBCAMEXP   is_Watchdog               (HCAM hCam, long lMode);
  // video out functions
  USBCAMEXP   is_SetPassthrough         (HCAM hCam, INT Source);

// ----------------------------------------------------------------------------
// alias functions for compatibility
// ----------------------------------------------------------------------------
  USBCAMEXP   is_InitBoard              (HCAM* phCam, HWND hWnd);
  USBCAMEXP   is_ExitBoard              (HCAM hCam);
  USBCAMEXP   is_InitFalcon             (HCAM* phCam, HWND hWnd);
  USBCAMEXP   is_ExitFalcon             (HCAM hCam);

  USBCAMEXP   is_GetBoardType           (HCAM hCam);
  USBCAMEXP   is_GetBoardInfo           (HCAM hCam, PBOARDINFO pInfo);
  USBCAMEXPUL is_BoardStatus            (HCAM hCam, INT nInfo, ULONG ulValue);
...
// ----------------------------------------------------------------------------
  ...
  USBCAMEXP   is_SetBrightness          (HCAM hCam, INT Bright);
  USBCAMEXP   is_SetContrast            (HCAM hCam, INT Cont);
  ...
  USBCAMEXP   is_SaveImageMem           (HCAM hCam, const IS_CHAR* File, char* pcMem, int nID);
  ...
  USBCAMEXP   is_SetImageSize           (HCAM hCam, INT x, INT y);
  USBCAMEXP   is_SetImagePos            (HCAM hCam, INT x, INT y);
  ...
  USBCAMEXP   is_SaveImage              (HCAM hCam, const IS_CHAR* File);
  ...
  USBCAMEXP   is_GetDC                  (HCAM hCam, HDC* phDC);
  USBCAMEXP   is_ReleaseDC              (HCAM hCam, HDC hDC);
  USBCAMEXP   is_UpdateDisplay          (HCAM hCam);
  USBCAMEXP   is_SetDisplaySize         (HCAM hCam, INT x, INT y);
  USBCAMEXP   is_SetDisplayPos          (HCAM hCam, INT x, INT y);

  USBCAMEXP   is_LockDDOverlayMem       (HCAM hCam, VOID** ppMem, INT* pPitch);
  USBCAMEXP   is_UnlockDDOverlayMem     (HCAM hCam);
  USBCAMEXP   is_LockDDMem              (HCAM hCam, VOID** ppMem, INT* pPitch);
  USBCAMEXP   is_UnlockDDMem            (HCAM hCam);
  USBCAMEXP   is_GetDDOvlSurface        (HCAM hCam, void** ppDDSurf);
  USBCAMEXP   is_SetKeyColor            (HCAM hCam, INT r, INT g, INT b);
  USBCAMEXP   is_StealVideo             (HCAM hCam, int Wait);
  USBCAMEXP   is_SetHwnd                (HCAM hCam, HWND hwnd);


  USBCAMEXP   is_SetDDUpdateTime        (HCAM hCam, INT ms);
  USBCAMEXP   is_EnableDDOverlay        (HCAM hCam);
  USBCAMEXP   is_DisableDDOverlay       (HCAM hCam);
  USBCAMEXP   is_ShowDDOverlay          (HCAM hCam);
  USBCAMEXP   is_HideDDOverlay          (HCAM hCam);
  ...
  USBCAMEXP   is_SetIO                  (HCAM hCam, INT nIO);
  USBCAMEXP   is_SetFlashStrobe         (HCAM hCam, INT nMode, INT nLine);
...
// ----------------------------------------------------------------------------
  ...
  // PixelClock
  USBCAMEXP is_GetPixelClockRange          (HCAM hCam, INT *pnMin, INT *pnMax);
  USBCAMEXP is_SetPixelClock               (HCAM hCam, INT Clock);
  ...
  // Set/Get Exposure
  USBCAMEXP is_GetExposureRange            (HCAM hCam, double *min, double *max, double *intervall);
  USBCAMEXP is_SetExposureTime             (HCAM hCam, double EXP, double* newEXP);
  // get frames per second
  USBCAMEXP is_GetFramesPerSecond          (HCAM hCam, double *dblFPS);

  // is_SetIOMask ( only uc480 USB )
  USBCAMEXP is_SetIOMask                   (HCAM hCam, INT nMask);
  ...
  // Edge enhancement
  USBCAMEXP is_SetEdgeEnhancement          (HCAM hCam, INT nEnable);
  ...
  // Hotpixel
  USBCAMEXP is_SetBadPixelCorrection       (HCAM hCam, INT nEnable, INT threshold);
  USBCAMEXP is_LoadBadPixelCorrectionTable (HCAM hCam, const IS_CHAR *File);
  USBCAMEXP is_SaveBadPixelCorrectionTable (HCAM hCam, const IS_CHAR *File);
  USBCAMEXP is_SetBadPixelCorrectionTable  (HCAM hCam, INT nMode, WORD *pList);

  // Memoryboard
  USBCAMEXP is_SetMemoryMode               (HCAM hCam, INT nCount, INT nDelay);
  USBCAMEXP is_TransferImage               (HCAM hCam, INT nMemID, INT seqID, INT imageNr, INT reserved);
  USBCAMEXP is_TransferMemorySequence      (HCAM hCam, INT seqID, INT StartNr, INT nCount, INT nSeqPos);
  USBCAMEXP is_MemoryFreezeVideo           (HCAM hCam, INT nMemID, INT Wait);
  USBCAMEXP is_GetLastMemorySequence       (HCAM hCam, INT *pID);
  USBCAMEXP is_GetNumberOfMemoryImages     (HCAM hCam, INT nID, INT *pnCount);
  USBCAMEXP is_GetMemorySequenceWindow     (HCAM hCam, INT nID, INT *left, INT *top, INT *right, INT *bottom);
  USBCAMEXP is_IsMemoryBoardConnected      (HCAM hCam, BOOL *pConnected);
  USBCAMEXP is_ResetMemory                 (HCAM hCam, INT nReserved);
  ...
  USBCAMEXP is_LoadParameters              (HCAM hCam, const IS_CHAR* pFilename);
  USBCAMEXP is_SaveParameters              (HCAM hCam, const IS_CHAR* pFilename);

  // new with driver version 1.14.0001
  USBCAMEXP is_GetGlobalFlashDelays        (HCAM hCam, ULONG *pulDelay, ULONG *pulDuration);
  USBCAMEXP is_SetFlashDelay               (HCAM hCam, ULONG ulDelay, ULONG ulDuration);
  // new with driver version 1.14.0002
  USBCAMEXP is_LoadImage                   (HCAM hCam, const IS_CHAR* File);

  // new with driver version 1.14.0008
  USBCAMEXP is_SetImageAOI                 (HCAM hCam, INT xPos, INT yPos, INT width, INT height);
  USBCAMEXP is_SetCameraID                 (HCAM hCam, INT nID);
  USBCAMEXP is_SetBayerConversion          (HCAM hCam, INT nMode);
  USBCAMEXP is_SetTestImage                (HCAM hCam, INT nMode);
  ...
  USBCAMEXP is_SetAOI                      (HCAM hCam, INT type, INT *pXPos, INT *pYPos, INT *pWidth, INT *pHeight);
  ...
  USBCAMEXP is_ConvertImage                (HCAM hCam, char* pcSource, int nIDSource, char** pcDest, INT *nIDDest, INT *reserved);
  USBCAMEXP is_SetConvertParam             (HCAM hCam, BOOL ColorCorrection, INT BayerConversionMode, INT ColorMode, INT Gamma, double* WhiteBalanceMultipliers);
  
  USBCAMEXP is_SaveImageEx                 (HCAM hCam, const IS_CHAR* File, INT fileFormat, INT Param);
  USBCAMEXP is_SaveImageMemEx              (HCAM hCam, const IS_CHAR* File, char* pcMem, INT nID, INT FileFormat, INT Param);
  USBCAMEXP is_LoadImageMem                (HCAM hCam, const IS_CHAR* File, char** ppcImgMem, INT* pid);
  ...
  USBCAMEXP is_SetLED                      (HCAM hCam, INT nValue);
  ...
  USBCAMEXP is_GetCaptureErrorInfo         (HCAM hCam, UC480_CAPTURE_ERROR_INFO *pCaptureErrorInfo, UINT SizeCaptureErrorInfo);
  USBCAMEXP is_ResetCaptureErrorInfo       (HCAM hCam);
...
#pragma pack( push, 1)
  ...
  typedef struct _UC480_COMPORT_CONFIGURATION
  {
    ...
    {
        BYTE    reserved_1[24];
        DWORD   dwRuntimeFirmwareVersion;
        BYTE    reserved_2[8];
        WORD    wTemperature;
        WORD    wLinkSpeed_Mb;
        BYTE    reserved_3[6];
    ...
    typedef struct S_IS_DEVICE_INFO
    {
        ...
        IS_DEVICE_INFO_CONTROL      infoDevControl;
        BYTE                        reserved[240];
...
#pragma pack(pop)
    ...
    USBCAMEXP is_GetEthDeviceInfo    (HCAM hCam, UC480_ETH_DEVICE_INFO* pDeviceInfo, UINT uStructSize);
    USBCAMEXP is_SetPersistentIpCfg  (HCAM hCam, UC480_ETH_IP_CONFIGURATION* pIpCfg, UINT uStructSize);
  ...
  typedef enum E_IMGSTAB_CMD
  {
    ...
    #define IS_BOOTBOOST_ID_MIN     1   
    #define IS_BOOTBOOST_ID_MAX     254 
    #define IS_BOOTBOOST_ID_NONE    0   
    #define IS_BOOTBOOST_ID_ALL     255 
    ...
    USBCAMEXP is_ImageBuffer(HCAM hf, UINT nCommand, void* pParam, UINT cbSizeOfParam);
